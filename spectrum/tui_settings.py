...
FIELDS = OrderedDict([
    ...
    ("use_database_framework", ("bool",)),  # <-- KB SEARCH TOGGLE
])
...
def run(stdscr):
    ...
    if "use_database_framework" not in config:
        config["use_database_framework"] = False
    ...
    while True:
        ...
        if key == curses.KEY_UP and row > 0:
            ...
            if field_type == "bool":
                ...
                status = f"{sel_key} toggled to {config[sel_key]}"
                continue
